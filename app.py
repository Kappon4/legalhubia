...
# --- NOVA FUNÇÃO: BUSCA ANTI-ALUCINAÇÃO (JURISPRUDÊNCIA REAL) ---
...
# --- CSS VISUAL (RESGATADO DO SEU CÓDIGO ORIGINAL) ---
def local_css():
...
if not st.session_state.logado:
    ...
    with col2:
        ...
        else: st.success("☁️ CONECTADO AO BANCO DE DADOS (SUPABASE)")
        ...
        with st.container(border=True):
            ...
            with tab_login:
                ...
                if c_reset.button("🆘 Reset Admin", use_container_width=True):
                    ...
                    except: st.error("Erro no reset")

            with tab_cadastro:
                new_user = st.text_input("Novo Usuário", key="cad_user")
                new_pwd = st.text_input("Nova Senha", type="password", key="cad_pwd")
                new_office = st.text_input("Escritório", key="cad_office")
                if st.button("CADASTRAR", use_container_width=True):
                    if new_user and new_pwd:
                        try:
                            run_query("INSERT INTO usuarios (username, senha, escritorio, creditos, plano) VALUES (%s, %s, %s, 10, 'starter')", (new_user, new_pwd, new_office))
                            st.success("Cadastrado! Faça Login.")
                        except: st.error("Usuário já existe.")
        st.markdown("<div style='text-align:center; margin-top:10px; color: #475569; font-size: 0.7rem; font-family: Rajdhani;'>SYSTEM V7.9 // SECURE</div>", unsafe_allow_html=True)
    st.stop()

# ==========================================================
# 5. SIDEBAR & MENU SUPERIOR (RESGATADO)
# ==========================================================
...
# --- MENU HORIZONTAL NO TOPO (VISUAL ANTIGO RESGATADO) ---
...
# ==========================================================
# LÓGICA DAS TELAS (COM NOVAS FUNÇÕES)
# ==========================================================
...
if menu_opcao == "📊 Dashboard":
    ...
    with r1:
        with st.container(border=True):
            ...
            st.markdown("Gere peças jurídicas completas com busca de jurisprudência real.")
            if st.button("ABRIR REDATOR"): st.session_state.navegacao_override = "✍️ Redator Jurídico"; st.rerun()
    with r2:
        with st.container(border=True):
            st.markdown("#### 🧮 CÁLCULOS")
            st.markdown("Trabalhista, Cível, Família, Tributário e Criminal.")
    ...
    if area_calc == "Trabalhista (CLT)":
        ...
        with tab_liq:
            ...
            if st.button("LIQUIDAR"):
                res = val * indice
                if juros == "1% a.m.": res *= 1.12 # Simulação
