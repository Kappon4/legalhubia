...
st.set_page_config(
    page_title="LegalHub Elite v9.0", 
    page_icon="⚖️", 
    layout="wide",
...
# ==========================================================
# 2. CONEXÃO COM BANCO DE DADOS (SILENT IPv4 FIX)
# ==========================================================
def get_ipv4_address(uri):
    """Resolve o endereço do banco para IPv4 numérico para evitar erros de IPv6."""
    try:
        result = urlparse(uri)
        hostname = result.hostname
        # Pega o IP numérico (ex: 34.123.45.6)
        ipv4 = socket.gethostbyname(hostname)
        return uri.replace(hostname, ipv4)
    except:
        return uri
...
def get_db_connection():
    ...
    try:
        return psycopg2.connect(DB_URI, sslmode='require', connect_timeout=3)
    except Exception:
        # Se falhar (erro IPv6), tenta o FIX IPv4 silenciosamente
        try:
            uri_v4 = get_ipv4_address(DB_URI)
            return psycopg2.connect(uri_v4, sslmode='require', connect_timeout=5)
...
def run_query(query, params=(), return_data=False):
    ...
    try:
        ...
        if conn: conn.close()
        # Removemos o st.error daqui para não poluir a tela com tentativas falhas
        return None

# ==========================================================
# 3. FUNÇÕES GERAIS (REDATOR, CÁLCULOS, ETC.)
# ==========================================================
...
def buscar_contexto_juridico(tema, area):
    fontes = {"Criminal": "site:stj.jus.br", "Trabalhista": "site:tst.jus.br", "Cível": "site:stj.jus.br"}
...
# ==========================================================
# 4. TELA DE LOGIN (COM REPARO E CORREÇÃO DE REDE)
# ==========================================================
...
if not st.session_state.logado:
    ...
    with col2:
        ...
        if CONEXAO_NUVEM:
             # Verifica conexão real silenciosamente
             if get_db_connection():
                 st.success("☁️ CONEXÃO SEGURA (ONLINE)")
             else:
                 st.error("⚠️ ERRO DE REDE: O Streamlit não conseguiu alcançar o Supabase.")
        else:
             st.warning("⚠️ MODO OFFLINE")

        with st.container(border=True):
            tab1, tab2, tab3 = st.tabs(["ENTRAR", "CADASTRAR", "🔧 SUPORTE"])
            ...
            with tab1:
                ...
                if st.button("LOGIN", use_container_width=True):
                    ...
                    if res is not None and not res.empty:
                        ...
                        st.rerun()
                    else: st.error("Dados inválidos ou erro de conexão.")
                
                if st.button("🆘 Resetar Admin"):
                    # Tenta inserir Admin
                    res = run_query("INSERT INTO usuarios (username, senha, escritorio, email_oab, creditos, plano) VALUES ('admin', 'admin', 'Master Office', 'adm@lh.com', 9999, 'full') ON CONFLICT (username) DO UPDATE SET senha='admin'")
                    if res:
                        st.success("Admin resetado! Use: admin / admin")
                    else:
                        st.error("Falha ao resetar. Clique em REPARAR BANCO na aba Suporte.")
            ...
            with tab3:
                st.warning("Use isso se estiver com problemas de login ou erros vermelhos.")
                if st.button("🛠️ REPARAR BANCO DE DADOS (RESET TOTAL)", use_container_width=True):
...
with st.sidebar:
    ...
    if st.session_state.usuario_atual == 'admin':
        with st.expander("⚙️ ADMIN"):
            ...
            user_to_edit = st.selectbox("Usuário", all_users['username'].unique() if all_users is not None else [])
            new_plan = st.selectbox("Novo Plano", ["starter", "full", "criminal", "trabalhista", "civil"])
...
if menu == "📊 Dashboard":
    ...
    pecas = ["Petição Inicial", "Contestação", "Réplica", "Recurso", "Habeas Corpus", "Mandado de Segurança", "Reclamação Trabalhista"]
