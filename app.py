...
# --- INSTALAÇÃO AUTOMÁTICA DE DEPENDÊNCIAS (CRÍTICO PARA NUVEM) ---
try:
    import psycopg2
except ImportError:
...
# ==========================================================
# 2. CONEXÃO COM BANCO DE DADOS (VIA SECRETS)
# ==========================================================
try:
    # Tenta pegar as credenciais do cofre do Streamlit (Seguro)
    DB_URI = st.secrets["DB_URI"]
    API_KEY_FIXA = st.secrets["GOOGLE_API_KEY"]
    USAR_SQLITE_BACKUP = False
except:
    # Se falhar (ex: rodando local sem config), avisa e usa modo offline
    st.warning("⚠️ Segredos não detectados. Usando modo SQLite temporário.")
    DB_URI = ""
    API_KEY_FIXA = "" 
...
def run_query(query, params=(), return_data=False):
    ...
    try:
        ...
        # Adaptação de sintaxe (SQLite = ? / Postgres = %s)
        if USAR_SQLITE_BACKUP:
...
def tentar_gerar_conteudo(prompt, api_key_val):
    ...
    if not chave: return "⚠️ Erro: API Key não configurada nos Secrets."
    ...
    return "❌ Erro na IA: Verifique sua Chave API ou conexão."
...
if not st.session_state.logado:
    st.markdown("<br><h1 style='text-align:center'>🛡️ LEGALHUB ELITE</h1>", unsafe_allow_html=True)
    
    if not USAR_SQLITE_BACKUP: 
        st.success("☁️ CONECTADO AO BANCO DE DADOS (SUPABASE)")
    else:
        st.warning("⚠️ MODO OFFLINE (SECRETS NÃO ENCONTRADOS)")
...
with st.sidebar:
    ...
    menu = st.radio("Navegação", ["Dashboard", "Redator IA", "📜 Contratos", "Calculadoras", "Cofre Digital"])
...
if menu == "Dashboard":
    ...
    with c1:
        tipo = st.selectbox("Peça", ["Petição Inicial", "Contestação", "Habeas Corpus", "Recurso", "Notificação"])
    ...
    tab1, tab2 = st.tabs(["Contrato", "Procuração"])
    ...
    with tab1:
        ...
        forma = c2.selectbox("Pagamento", ["À Vista", "Parcelado", "No Êxito"])
        objeto = st.text_area("Objeto do Serviço")
        
        if st.button("Criar Contrato"):
            with st.spinner("Redigindo contrato..."):
                prompt = f"Redija contrato de honorários. Cliente: {cliente}, CPF {cpf}. Valor: R$ {valor}. Forma: {forma}. Objeto: {objeto}. Contratado: {st.session_state.escritorio_atual}. Incluir cláusulas de foro e multa."
                res = tentar_gerar_conteudo(prompt, None)
                st.markdown(res)
                if "❌" not in res:
                    st.download_button("Baixar DOCX", gerar_word(res), "Contrato.docx")
                    run_query("INSERT INTO documentos (escritorio, data_criacao, cliente, area, tipo, conteudo) VALUES (%s, %s, %s, %s, %s, %s)", 
                             (st.session_state.escritorio_atual, str(date.today()), cliente, "Contratos", "Contrato Honorários", res))

    with tab2:
        out = st.text_input("Outorgante (Cliente)")
        dados = st.text_input("Dados Completos (Nac, Est. Civil, Prof, RG, CPF, Endereço)")
        if st.button("Criar Procuração"):
            prompt = f"Redija Procuração Ad Judicia. Outorgante: {out}, {dados}. Outorgado: {st.session_state.escritorio_atual}."
            res = tentar_gerar_conteudo(prompt, None)
            st.markdown(res)
