...
st.set_page_config(page_title="LegalHub Elite v8.8", page_icon="⚖️", layout="wide")
...
# ==========================================================
# 2. CONEXÃO COM BANCO (COM LOG DE ERRO VISUAL)
# ==========================================================
def get_db_connection():
    if CONEXAO_NUVEM:
        try:
            ...
            st.error(f"Erro conexão técnica: {e}")
            return None
    else:
        import sqlite3
...
def run_query(query, params=(), return_data=False):
    ...
    try:
        ...
        if conn: conn.close()
        # AQUI O SEGREDO: MOSTRAR O ERRO REAL NA TELA
...
# ==========================================================
# 4. TELA DE LOGIN (COM FERRAMENTA DE REPARO)
# ==========================================================
            ...
            with tab1:
                ...
                if st.button("LOGIN", use_container_width=True):
                    ...
                    if res is not None and not res.empty:
                        ...
                        st.rerun()
                    else: st.error("Dados inválidos ou usuário inexistente.")
                
                if st.button("🆘 Resetar Admin"):
                    # Tenta inserir, se falhar mostra erro
                    if run_query("INSERT INTO usuarios (username, senha, escritorio, email_oab, creditos, plano) VALUES ('admin', 'admin', 'Master Office', 'adm@lh.com', 9999, 'full') ON CONFLICT (username) DO UPDATE SET senha='admin'"):
                        ...
                        st.error("Falha ao resetar. Vá na aba SUPORTE e clique em REPARAR.")
            ...
            with tab3:
                ...
                if st.button("🛠️ REPARAR BANCO DE DADOS (RESET TOTAL)", use_container_width=True):
                    with st.spinner("Limpando e recriando tabelas..."):
                        ...
                        st.success("Banco reparado com sucesso! Tente logar com admin/admin.")
...
if menu == "📊 Dashboard":
    ...
    pecas = ["Petição Inicial", "Contestação", "Réplica", "Recurso", "Habeas Corpus", "Mandado de Segurança", "Reclamação Trabalhista", "Agravo"]
