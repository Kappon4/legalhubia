...
# --- CSS AVANÇADO COM BACKGROUND FLUTUANTE ---
def local_css():
    # Tenta carregar a imagem de fundo (unnamed.jpg)
    bg_image_b64 = get_base64_of_bin_file("unnamed.jpg")
    
    # CSS para o Background Flutuante (Se a imagem existir)
    bg_css = ""
    if bg_image_b64:
        bg_css = f"""
        ...
        .stApp::before {{
            ...
            width: 60%; /* Tamanho da logo no fundo */
            height: 60%;
            background-image: url("data:image/jpeg;base64,{bg_image_b64}");
            background-size: contain;
            background-repeat: no-repeat;
            background-position: center;
            opacity: 0.08; /* Opacidade bem baixa para não atrapalhar */
    ...
    <style>
        ...
        /* INSERÇÃO DO BACKGROUND DINÂMICO */
        {bg_css}
        ...
        /* PLAN CARDS */
        .plan-card {{
            background: rgba(15, 23, 42, 0.85); /* Fundo um pouco mais opaco para ler sobre a imagem */
...
if api_key:
    ...
    # CORREÇÃO CRÍTICA DO MODELO (SEM PREFIXO 'models/')
    mod_escolhido = "gemini-1.5-flash"
