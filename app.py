...
# ==========================================================
st.set_page_config(
    page_title="LegalHub Elite v7.7", 
    page_icon="⚖️", 
    layout="wide",
...
# ==========================================================
# 4. LOGIN & CADASTRO (ATUALIZADO v7.7)
# ==========================================================
try:
    if USAR_SQLITE_BACKUP:
    ...
    with col2:
        ...
        with st.container(border=True):
            ...
            with tab_cadastro:
                ...
                new_email = st.text_input("Email OAB (Opcional)", key="cad_email")
                ...
                if st.button("CADASTRAR E ENTRAR", use_container_width=True):
                    if new_user and new_pwd and new_office:
                        ...
                        if check is not None and not check.empty:
                            ...
                            # Cadastra
                            try:
                                q_insert = "INSERT INTO usuarios (username, senha, escritorio, email_oab, creditos, plano) VALUES (%s, %s, %s, %s, %s, %s)"
                                run_query(q_insert, (new_user, new_pwd, new_office, new_email, 10, 'starter'))
                                st.success("Cadastro realizado! Fazendo login...")
                                time.sleep(1)
                                st.session_state.logado = True
                                st.session_state.usuario_atual = new_user
                        ...
                        st.warning("Preencha Usuário, Senha e Escritório.")

            st.markdown("<div style='text-align:center; margin-top:10px; color: #475569; font-size: 0.7rem; font-family: Rajdhani;'>SYSTEM V7.7 // SECURE</div>", unsafe_allow_html=True)
    st.stop()
...
st.markdown("---")
st.markdown("<center>🔒 LEGALHUB ELITE v7.7 | POSTGRESQL SECURE</center>", unsafe_allow_html=True)
