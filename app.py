...
# ==========================================================
# 1. DIAGNÓSTICO DE AMBIENTE (O CÓDIGO COMEÇA AQUI)
# ==========================================================

# Teste 1: A biblioteca do banco está instalada?
try:
    import psycopg2
    LIB_INSTALADA = True
except ImportError:
...
# ==========================================================
st.set_page_config(
    page_title="LegalHub Elite v8.6", 
    page_icon="⚖️", 
    layout="wide",
...
# ==========================================================
# 3. CONEXÃO COM BANCO DE DADOS (COM DIAGNÓSTICO)
# ==========================================================
# Teste 2: A senha está configurada no Streamlit Cloud?
try:
...
# Variável Mestra: Só conecta se tiver a Lib E a Senha
CONEXAO_NUVEM = (LIB_INSTALADA and SECRETS_OK)
...
def get_db_connection():
    if CONEXAO_NUVEM:
        try:
            ...
            # Se chegou aqui, tem lib e tem secrets, mas a senha pode estar errada
            st.error(f"ERRO DE CONEXÃO COM SUPABASE: {e}")
            return None
    else:
        # Modo Offline (Memória) para não travar o app visualmente
        import sqlite3
...
if not CONEXAO_NUVEM:
    try:
        ...
        c.execute("INSERT OR IGNORE INTO usuarios (username, senha, escritorio, creditos, plano) VALUES ('admin', 'admin', 'Master Office', 9999, 'full')")
        conn.commit()
    except: pass
else:
    # Postgres Nuvem
...
# ==========================================================
# 5. LOGIN (COM DIAGNÓSTICO VISUAL)
# ==========================================================
...
if not st.session_state.logado:
    ...
    with col2:
        ...
        # --- DIAGNÓSTICO DE ERRO VISUAL ---
        if not LIB_INSTALADA:
            st.error("❌ ERRO CRÍTICO 1: Biblioteca 'psycopg2' não encontrada.")
            st.info("💡 SOLUÇÃO: Crie o arquivo requirements.txt no GitHub com 'psycopg2-binary'.")
        elif not SECRETS_OK:
            st.error("❌ ERRO CRÍTICO 2: Secrets não configurados.")
            st.info("💡 SOLUÇÃO: Configure DB_URI e GOOGLE_API_KEY no painel do Streamlit Cloud.")
        elif not CONEXAO_NUVEM:
            st.warning("⚠️ MODO OFFLINE ATIVADO (Erro na conexão com Banco)")
        else:
        ...
        with st.container(border=True):
            ...
            with tab_login:
                ...
                if c_entrar.button("LOGIN", use_container_width=True):
                    # Login de emergência para testar layout mesmo offline
                    if not CONEXAO_NUVEM and user_input == 'admin' and pwd_input == 'admin':
                        st.session_state.logado = True
                        st.session_state.usuario_atual = 'admin (offline)'
                        st.session_state.plano_atual = 'full'
                        st.session_state.escritorio_atual = 'Localhost'
                        st.rerun()
                ...
                if c_reset.button("🆘 Reset Admin", use_container_width=True):
                    if not CONEXAO_NUVEM:
                        st.error("Impossível resetar admin em modo offline.")
                    elif run_query("INSERT INTO usuarios (username, senha, escritorio, email_oab, creditos, plano) VALUES ('admin', 'admin', 'Master Office', 'adm@lh.com', 9999, 'full') ON CONFLICT (username) DO UPDATE SET senha = 'admin'"):
            ...
            with tab_cadastro:
                ...
                if st.button("CADASTRAR", use_container_width=True):
                    if not CONEXAO_NUVEM:
                        st.error("Erro: Banco de Dados desconectado. Não é possível cadastrar.")
        ...
        st.markdown("<div style='text-align:center; margin-top:10px; color: #475569;'>SYSTEM V8.6 // DIAGNOSTIC MODE</div>", unsafe_allow_html=True)
    st.stop()
...
st.markdown("---")
st.markdown("<center>🔒 LEGALHUB ELITE v8.6 | POSTGRESQL SECURE</center>", unsafe_allow_html=True)
